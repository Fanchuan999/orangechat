...
fun SettingProactiveMessagePage(vm: SettingVM = koinInject()) {
    ...
    var showAutonomousActivityRiskDialog by remember { mutableStateOf(false) }
    ...
    if (showAutonomousActivityRiskDialog) {
        RiskConfirmDialog(
            ...
            onConfirm = {
                ...
                val updated = proactive.copy(autonomousActivity = proactive.autonomousActivity.copy(enabled = true))
                vm.updateSettings(settings.copy(proactiveMessageSetting = updated))
            },
            onDismiss = { showAutonomousActivityRiskDialog = false },
        )
    }
            ...
            item {
                ...
                CardGroup(title = { Text("空闲时自己找点事") }) {
                    ...
                    if (proactiveSetting.idleExploreEnabled) {
                        ...
                        item(
                            headlineContent = { Text("允许空闲时自动对外活动") },
                            supportingContent = {
                                Text(
                            ...
                            trailingContent = {
                                Switch(
                                    checked = proactiveSetting.autonomousActivity.enabled,
                                    onCheckedChange = { enabled ->
                                        if (enabled) {
                                            showAutonomousActivityRiskDialog = true
                                        } else {
                                            val updated = proactiveSetting.copy(
                                                autonomousActivity = proactiveSetting.autonomousActivity.copy(enabled = false),
                                            )
                                            vm.updateSettings(settings.copy(proactiveMessageSetting = updated))
                                        }
                                    },
                        ...
                        if (proactiveSetting.autonomousActivity.enabled) {
                            ...
                            item(
                                ...
                                supportingContent = {
                                    if (autonomousActivityRecords.isEmpty()) {
                                        Text("暂无记录。这里最多保留最近 50 条安全摘要；不会显示工具参数、账号、密钥、Cookie 或 MCP 地址。")
                                    } else {
                                        Column(verticalArrangement = Arrangement.spacedBy(4.dp)) {
                                            autonomousActivityRecords.forEach { record ->
                                                val time = record.createdAt.atZone(ZoneId.systemDefault())
                                                    .format(DateTimeFormatter.ofPattern("M/d HH:mm"))
                                                Text(
                                                    "$time · ${record.family.displayName()} · ${record.status.displayName()}" +
                                                        "${record.toolName?.let { " · $it" }.orEmpty()}\n${record.summary}",
                                                )
                                            }
                                        }
                                    }
...
private fun AutonomousActivityStatus.displayName(): String = when (this) {
    AutonomousActivityStatus.STARTED -> "已启动"
    AutonomousActivityStatus.COMPLETED -> "已完成"
    AutonomousActivityStatus.FAILED -> "失败"
    AutonomousActivityStatus.DECLINED -> "已跳过"
    AutonomousActivityStatus.UNAVAILABLE -> "不可用"
}
